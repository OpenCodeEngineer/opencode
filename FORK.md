...
| **Enterprise wellknown SSRF** | HIGH | Wellknown auth entry configured | If `.well-known/opencode` is compromised, auth headers can be forwarded to attacker URL |
...
4. Enterprise users: validate `remote_config.url` domain in wellknown responses before deploying.
